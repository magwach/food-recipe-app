...
export default function Favourites() {
    ...
    return (
        <div className={`${favourites.length ? 'grid grid-cols-3 gap-2 place-items-center h-auto' : 'text-center'}`}>
            {
                favourites.length ? favourites.map(item => {
                    return (
                        <div className="flex flex-col p-4 bg-[#f8f8f8] h-115 lg:h-150 rounded-lg cursor-pointer ">
                            ...
                            <button className="w-full py-2 text-sm lg:text-2xl font-medium text-[#f8f8f8] bg-[#1c1c1c] rounded-xl mt-3 lg:mt-11 text-center tracking-wider cursor-pointer" onClick={() => handleFavourites(item)}>Remove from favourites</button>
                        </div>
                    )
                })
